...
export function renderAnswerOutput(text) {
...
export function renderCitationsDetails(technical) {
    if (!technical)
        return "";
...
export function renderTechnicalDetails(technical) {
    if (!technical)
        return "";
    ...
    <div class="content">
      <p class="muted">Route: <b>${escapeHtml(technical.route ?? "?")}</b>${technical.alpha != null ? ` (alpha=${technical.alpha})` : ""} · Revisions: ${technical.answer_revision_count}</p>
...
export function renderQueryState(state) {
